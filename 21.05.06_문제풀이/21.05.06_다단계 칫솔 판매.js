...
자신을 조직에 참여시킨 추천인에 연결되어 피라미드 식의 구조를 이루고 있습니다. 조직의 이익 분배 규칙은 간단합니다. 
...
*/
function cal(obj, sell, my){
    if(obj[sell].p == "center"){
        if(my*0.1 < 0){
         //   console.log(my, sell, obj[sell].money);
            obj[sell].money = obj[sell].money + my;
            return;
        }
        obj[sell].money = obj[sell].money + Math.ceil(my*0.9);
        return;
    }
    if(my*0.1 < 0){
        obj[sell].money = obj[sell].money + my;
    ...
    obj[sell].money = obj[sell].money + Math.ceil(my*0.9);
    cal(obj,obj[sell].p, my - Math.ceil(my*0.9));  
}
function solution(enroll, referral, seller, amount) {
    ...
    for(let i = 0; i < enroll.length; i++){
     ...
     //   obj[enroll[i]].moneny = 0;
    }
    while(seller.length > 0){
       // console.log("Tlfgod");
        let s = seller.shift();
        let m = amount.shift();
        cal(obj, s, m*100);
