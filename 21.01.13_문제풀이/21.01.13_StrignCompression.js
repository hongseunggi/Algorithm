...
function solution(s) {
    var answer = 0;
    answer = 1000
    for (let x = 1; x < Math.floor(s.length/2)+2; x++){
        var string=''
        var y=0
        var count=1
        while (y < s.length){
           //console.log(s.substring(y,y+x),x,s)
            if(s.substring(y,y+x)==s.substring(y+x,y+x+x)){
                count+=1;
            }
            else if(count<=1){
                string+=s.substring(y,y+x);
                count=1;
            }
            else{
                string+=String(count)+s.substring(y,y+x);
                count=1;
            }
            y+=x;
            //print(string)
