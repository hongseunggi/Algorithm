...
function solution(a, edges) {
    if(a.reduce((v,i) => v+i, 0) != 0){
        return -1;
    }
    let answer = 0n; // bigint, 일반 number 사용시 계산이 잘못되는 경우 발생
    let tree = Array.from({length : a.length}, () => []);
    let visit = Array.from({length : a.length}, () => false);
    edges.map(v =>{
        let [start, end] = v;
        tree[start].push(end);
        tree[end].push(start);
...
//    console.log(tree);
    // stack을 이용한 dfs, 재귀 사용시 자바스크립트 특성상 런타임 에러 발생
    let stack = [[0,-1]];
