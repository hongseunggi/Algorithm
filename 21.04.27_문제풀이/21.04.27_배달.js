...
function solution(N, road, K) {
    ...
    let cost = Array.from({length: N}, () => Array(N).fill(500001));
    road.map(v => {
        let [start, end, weight] = v;
        graph[start-1][end-1] = 1;
        graph[end-1][start-1] = 1;
        if(cost[start-1][end-1] > weight){
            cost[start-1][end-1] = weight;
        }
        if(cost[end-1][start-1] > weight){
            cost[end-1][start-1] = weight;
        }
        //cost[end-1][start-1] = v[2];
        graph[start-1][start-1] = 0;
        graph[end-1][end-1] = 0;
        cost[start-1][start-1] = 0;
        cost[end-1][end-1] = 0;
    })
 //   console.log(graph, cost);
    for(let i = 0; i < N; i++){
        for(let x = 0; x < N; x++){
            for(let y = 0; y < N; y++){
                if(graph[x][i] == 1 && graph[i][y] == 1){
                    graph[x][y] = 1;
                    //   console.log(i,x,y);
                    if(cost[x][y] == 0){
                        continue;
                    }
                    if(cost[x][y] > cost[x][i]+cost[i][y]){
                        cost[x][y] = cost[x][i]+cost[i][y];    
                    }
                }
            }
        }
    }
    answer = cost[0].filter(v => v <= K);
 //   console.log(answer);
